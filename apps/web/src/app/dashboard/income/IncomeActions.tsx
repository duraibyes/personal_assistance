...
export default function IncomeActions({ incomeId, token }: { incomeId: string; token: string }) {
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true)
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api'}/incomes/${incomeId}`, {
        method: 'DELETE',
  ...
  return (
    <>
      <div className="flex items-center justify-end gap-2">
        <Link
          href={`/dashboard/income/${incomeId}/edit`}
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-background/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm text-left">
          <div className="bg-card border border-border p-6 rounded-2xl max-w-sm w-full shadow-2xl">
            <h3 className="text-lg font-bold text-foreground mb-2">Delete Income?</h3>
            <p className="text-muted-foreground text-sm mb-6 whitespace-normal">Are you sure you want to remove this income record? This cannot be undone.</p>
